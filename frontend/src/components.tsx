...
export function Topbar({ onMenu, onSearch, onTheme }: { onMenu: () => void; onSearch: () => void; onTheme: () => void }) {
  ...
  return <header className="topbar">
    ...
    <div className="topbar-actions"><button className="icon-button" aria-label="検索" onClick={onSearch}><SearchIcon /></button><button className="icon-button" aria-label="テーマ切替" onClick={onTheme}><SunIcon /></button><span className="avatar">創</span></div>
  </header>
}
...
export function SessionRail({ onCreateSession }: { onCreateSession: () => void }) {
  ...
  return <aside className="session-rail panel-scroll">
    <div className="rail-heading session-heading"><span>SESSIONS</span><button className="mini-button" aria-label="Sessionを追加" onClick={onCreateSession}><PlusIcon size={15} /></button></div>
